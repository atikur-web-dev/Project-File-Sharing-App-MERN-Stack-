...
// Extracted wrapper type to reduce endpoint boilerplate
...
const extractUser = (
  ...
  if (res.data.success && res.data.data?.user) return res.data.data.user;
...
export const getCurrentUserApi = async (): Promise<User | null> => {
  try {
    ...
    // Axios interceptor or logger should track actual 5xx server errors here
    return null;
  }
};
